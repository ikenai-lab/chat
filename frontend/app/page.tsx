...
export default function Home() {
  ...
  useEffect(() => {
    ...
    debounceTimer.current = setTimeout(() => {
        if (activeChatId) {
            fetch(`http://localhost:8000/api/v1/sessions/${activeChatId}/parameters`, {
                method: 'PUT',
  ...
  useEffect(() => {
    if (activeChatId) {
      const fetchMessages = async () => {
        setIsLoading(true);
        try {
          const response = await fetch(`http://localhost:8000/api/v1/sessions/${activeChatId}/messages`);
  ...
  const handleModelChange = async (modelName: string) => {
    ...
    try {
      const response = await fetch("http://localhost:8000/api/v1/models/load", {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const handleSendMessage = async (prompt: string, isRegeneration = false) => {
    ...
    if (!isRegeneration) {
        setActiveMessages(prev => [...prev, { role: "user", content: prompt }]);
    }
    ...
    try {
      ...
      if (shouldGenerateTitle) {
        await fetch("http://localhost:8000/api/v1/generate-title", {
            method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
        await fetch(`http://localhost:8000/api/v1/sessions/${chatToDelete.id}`, { method: "DELETE" });
  ...
  const handleConfirmDeleteModel = async () => {
    ...
    try {
        await fetch(`http://localhost:8000/api/v1/models/${modelToDelete}`, { method: "DELETE" });
  ...
  const handleSavePrompt = async (prompt: Omit<SystemPrompt, 'id'> & { id?: number }) => {
    try {
        ...
        const url = isUpdate ? `http://localhost:8000/api/v1/prompts/${prompt.id}` : "http://localhost:8000/api/v1/prompts";
  ...
  const handleDeletePrompt = async (promptId: number) => {
    try {
        const response = await fetch(`http://localhost:8000/api/v1/prompts/${promptId}`, { method: "DELETE" });
  ...
  const handleSystemPromptChange = async (promptId: number | null) => {
    ...
    try {
        await fetch(`http://localhost:8000/api/v1/sessions/${activeChatId}/prompt`, {
            method: "POST", headers: { "Content-Type": "application/json" },
